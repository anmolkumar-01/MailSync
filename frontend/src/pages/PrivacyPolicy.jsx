...
function PrivacyPolicy() {
  ...
  return (
    // 1. A container that defines the "page" area below the fixed header.
    // - `h-screen` makes it fill the viewport height.
    // - `pt-24` creates a 96px gap at the top, pushing all content down to clear your header.
    // - `pb-6` adds some padding at the bottom for better spacing on all screen sizes.
    // - `overflow-hidden` is a safeguard to prevent any accidental page scroll.
    <div className="w-full h-screen overflow-hidden bg-slate-50/50 px-4 sm:px-6 pt-24 pb-6">
      
      {/* 2. A new inner container to center the card. */}
      {/* - `h-full` and `w-full` make it fill the "safe area" defined above. */}
      {/* - `flex` with `items-center` and `justify-center` perfectly centers the card. */}
      <div className="h-full w-full flex items-center justify-center">

        {/* 3. The main card. */}
        {/* - `max-h-full` is the key change. It tells the card to never be taller than its parent (the safe area). */}
        {/* - This ensures the card fits on the screen without causing a browser scrollbar. */}
        <main className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-4xl max-h-full flex flex-col">

          {/* 4. The card's header. It takes up only the space it needs. */}
          <header className="p-6 border-b border-slate-200">
            <h1 className="flex items-center gap-3 text-2xl sm:text-3xl font-bold text-slate-800">
          ...
          {/* 5. The scrollable content area. This will scroll internally as designed. */}
